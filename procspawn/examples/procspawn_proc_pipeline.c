...
int main(int argc, char **argv)
{
	...
	for (i = 0; i <= n; i++) {	
		...
		//EC_ERRNO( (pe->p->pid = proc_spawn(pe->cmd, NULL, pe->p->pipes)) == -1, EXIT, perror(pe->cmd));
		EC_ERRNO( (p[i].pid = proc_spawn(*arg, arg, p[i].pipes, 0)) == -1, EXIT, perror(cmd[i]) );
		arg_free(&arg);
		...
		for (i = 0; i <= n; i++)
		{
			...
			{	
				...
				if (closed)
				{
					...
					p[i].pipes[1] = -1;
					if (i != n)
					{ /* close stdin of next process, no more data to write */
						close(p[i + 1].pipes[0]);
						p[i + 1].pipes[0] = -1;
					}
					else if (i == n)
